...
**本 skill 定位 = 人机协作载体（逐段协作，非 AI 独立成稿）**——此为定位声明，非能力缺陷标记。AI 草拟、用户逐段裁定定稿。

它提供的是**写作规范与结构框架 + 逐段协作规程**（问题怎么拆、方法论条目怎么引、段落按什么结构组织、投行语言的词汇句法怎么用、口径卡/漂移检测/敏感即停怎么走）。使用时须把握：
...
└── references/         # 写作规范（按文档类型组织）
    ...
    ├── methods-guide.md     # 方法论库使用指引（含外部引用路径清单）
    ├── paragraph-collab.md  # 逐段协作模式（含 §0.0 协作模式路由）
    ├── delivery-chain.md    # 交付链路与通道详规（格式落地串联全文/工具说明/依赖声明/L2 接口/边界详规）
    └── context-rules.md     # 上下文管理与降级
```

## 📌 近期更新

- **2026-09-20 · v0.14.6**：**方法论库引用面对齐库实况**——① 行业版路径由 `行业方法论/合并版/`（不存在）改为**扁平 `行业方法论_{类名}.md`** ② 单案路径补 `单案/` 前缀、案名一律用证券简称、无单案文件案路由改为 `cases_md/` ③ **新增语言专项入口**：招股书段落范式走「附：P 系列」→ `分卷/投行语言专项_招股书PL系列_卷N.md` ④ 最小起步结构 4 → **3 文件**（`编号登记表.md` 已于 2026-09-01 退役）⑤ 库规模数字更正为 **W 357 ／ P 442 ／ 单案 53**，并统一改为「以库内索引标注为准」防再次过期
...
- **2026-09-15 · v0.14.0**：**逐段协作模式落地**（从定位声明到操作规程）——新增 [paragraph-collab.md](references/paragraph-collab.md)（状态双文件：口径卡+写作进度、口径漂移检测协议、敏感词族识别即停、段后自检三档、交付四件套）；SKILL.md 定位声明改写 + §0 协作模式路由；writing-style.md 增「写前红线」节（禁词单一事实源 = doc-review sensitive_terms.json，写侧只放指针+心法）
