...
- **2026-09-20 · v0.14.6**：**方法论库引用面对齐库实况**——① 行业版路径由 `行业方法论/合并版/`（不存在）改为**扁平 `行业方法论_{类名}.md`** ② 单案路径补 `单案/` 前缀、案名一律用证券简称、无单案文件案路由改为 `cases_md/` ③ **新增语言专项入口**：招股书段落范式走「附：P 系列」→ `分卷/投行语言专项_P系列_卷N.md` ④ 最小起步结构 4 → **3 文件**（`编号登记表.md` 已于 2026-09-01 退役）⑤ 库规模数字更正为 **W 357 ／ P 442 ／ 单案 53**，并统一改为「以库内索引标注为准」防再次过期
