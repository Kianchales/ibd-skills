...
- **读取面分册 ＋ 提取脚本：把「读含修订/批注的 Word」下沉为可执行细则（承 R-0033 · 2026-09-24 用户当场定规则）**：判据正文已在 `docs/CONVENTIONS.md` §一 判据集「读取面 · 含修订/批注的 Word」行（**单一事实源，本包只回指针、不复制判据正文**）；本版补**操作侧**：
  - 新增 **`references/read-docx.md`**——三口径（**先探测** `w:ins`／`w:del`／`word/comments.xml` → 按「**接受全部修订 ＋ 剔除全部批注**」取终稿 → **输出标注**「该文件原件含修订/批注」）＋ 六条实现要点 ＋ 已知不覆盖 ＋ 加载时机；`SKILL.md`「使用流程」首部**步骤级内联**指向本册（可达性不依赖「资源索引」节）。
  - 新增 **`scripts/extract_final_text.py`**（**只读**，`zipfile` ＋ `xml.etree` 零第三方依赖，与「解压即跑」承诺一致）：`<docx> [--check] [-o out.txt] [--json]`；`w:ins` 内容**计入**、`w:del` **子树整棵排除**（含其中嵌套 `w:ins`）、批注**不进正文**、`w:tab`／`w:br` 归一化、跳过 `mc:Fallback`（防文本框内容与 `mc:Choice` 重复计入）；退出码 0＝成功／2＝用法或环境错误／3＝无正文可提取。
  - **双测**：新增 `scripts/tests/test_extract_final_text.py` **12 项**——阳性 5（ins 计入·delText 排除／嵌套 ins 一并排除／批注不进正文／tab·br 归一化／标注计数）＋ 阴性 2（无修订文件**输出不变味**、**不误报**标记）＋ 接口与安全 5（`--check` 不出正文／`--json` 字段齐备／`-o` 落盘为 LF／**源文件 sha256 不变**（只读证明）／坏输入 → 退出码 2）。
  - **动因（实证）**：审核问询回复原件含 `w:ins` **2,017**／`w:del` **2,979**／批注 **100**，直接取 `paragraph.text` **漏 17,242 字（正文 18.5%）且不报错**，致行业／法律／财务**三线同时误判**——「漏字不告警」是本条最贵的失败模式。
  - **同批文档校正**：`references/workflow.md` 的「脚本 × 场景」命令表补本脚本行；自测枚举由「四份／56 项」校正为**七份／120 项**（实测：10／22／11／18／26／21／12，**宣称数 ＝ 磁盘数**）。
  - **边界（不动既有语义）**：`revisions.md`（产出修订稿）与批注产出规范**均未改**；本册只管**读源件**——包内校验脚本（`check_content.py` 等）读的是**产物**、对照物不同，**未纳入**本册（若要扩面须另立判据）。

## [0.24.0] - 2026-09-24

- **新增核对项 `punct_usage`「标点用法」（规范源＝GB/T 15834—2011《标点符号用法》）**：把国标中**三项可机械判定**的规则纳入文字层门禁——① **数值范围起止用浪纹线 `~`**（§4.13.3.2 b；短横线只用于连接**号码**（门牌/电话/年月日，§4.13.3.1 b）与编号、化合物、复合名词、产品型号 ⇒ 故「2011-02-15」「表2-8」「WZ-10」「盎格鲁-撒克逊人」为**正确**用法、**不报**）；② **省略号不与「等」「等等」并用**（附录 A.9.2）；③ **图／表的短语式说明文字末尾不用句号**（附录 A.1）。**引号内引用原文一律豁免**（引用须照录，不得按规范改写他方原文）。
  - **严重度分工**：②③ → **MEDIUM**（计入 PASS/FAIL）；① → **LOW（仅提示、不阻断交付）**——实测实务语料该写法基数极大（库内百分之区间 `-` **1132** 处 vs `~` **46** 处），判阻断会致「逢查必红 ＝ 等于没有门禁」（同 R-0015 教训）。
  - **未纳入**：顿号/逗号/分号层次、书名号适用范围 —— 需语义判断，机械判必误报；留在方法论库 `S-010117–S-010119` 供写作时查阅（「库＝写前查、门禁＝交付前扫」两层分工）。
  - **双处同源的实现改进**：`deliver_gate.py` 的「标点用法」项**直接 import 复用** `content_text.check_punct_usage`（不再双写正则）；顺带修正 gate 侧一处**静默失效**——原先把整篇文本当单一 item 传入，致依赖**行首**匹配的表下注判据恒不触发（已改为按行切分）。
  - **同步 6 环**：① 规则本体 `references/rules.md` 三·8；② 检测脚本 `content_text.py` ＋ `deliver_gate.py`（import 复用）；③ 核对项名 —— `CHECK_REGISTRY`（文字项 7→8、总项 10→11）＋ `content_text.py` 模块 docstring ＋ SKILL.md 核对项数；④ 写作侧预防 `ibd-doc-write/references/writing-style.md` 写前红线；⑤ 本条目 ＋ 版本 bump ＋ README 变更摘要；⑥ 测试。
  - **测试**：`test_punct_usage.py` **15 例（6 阳性 ＋ 9 阴性）**，覆盖日期/表号/化合物/产品型号/外来语连字符**不误报**、引号内豁免、已用浪纹线不重复报；另跑 `deliver_gate.py --md` 端到端探针（MED 2 ＋ LOW 1 命中）。
