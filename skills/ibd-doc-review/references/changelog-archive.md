# Changelog 归档（0.1.0 – 0.14.1）

本文件收录 `ibd-doc-review` **0.15.0 之前**的版本记录（2026-08-25 起 至 2026-09-06 止，共 24 个版本）。
**0.15.0 及之后**的版本记录见主文件 [`../CHANGELOG.md`](../CHANGELOG.md)。

> 切分时逐字节搬运，未做任何内容改动。归档依据：主文件单文件体积过大（审计项 P3-⑨）。
...
- **references/annotations.md §3 去团队化**：编号前缀由「按角色固定映射（J=财务复核人 / L=法律复核人 / I=行业复核人 / Z=主理人）」固定映射 → 「复核流程自定义代号（1-2 大写字母，如 J=财务复核人），清单条目以 `code` 字段提供，规范不绑定具体人名」
